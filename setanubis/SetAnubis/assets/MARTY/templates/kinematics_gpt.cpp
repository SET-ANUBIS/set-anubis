...
//                 *(m_params->realParams.at(ss.str())) = m_calculator->get_momentum(i - 1) * m_calculator->get_momentum(j - 1);
//         }
//     }
// }
...
void Kinematics::set_ps_ind(std::function<bool(param_t*, const std::vector<double>&)> &ps_ind) {
    m_ps_ind = ps_ind;
}
...
bool Kinematics::is_point_valid() {
    return m_ps_ind(m_params, m_calculator->get_masses());
}

bool Kinematics::DEFAULT_PS_IND_13(param_t *p, const std::vector<double>& masses) { 
    double t = std::pow(masses[0], 2) - std::pow(masses[1], 2) - 2 * p->s_12;
    double s0 = (t + std::pow(masses[2], 2) - std::pow(masses[3], 2)) * (t + std::pow(masses[0], 2) - std::pow(masses[1], 2)) / (4 * t);
    double ds = std::pow(masses[0], 2) 
        * std::sqrt(1 - 2 * (t + std::pow(masses[1], 2)) / std::pow(masses[0], 2) + std::pow((t - std::pow(masses[1], 2)) / std::pow(masses[0], 2), 2))
        * std::sqrt(1 - 2 * (std::pow(masses[2], 2) + std::pow(masses[3], 2)) / t + std::pow((std::pow(masses[2], 2) - std::pow(masses[3], 2)) / t, 2)) / 4;
    
    
    //TODO : THEO changes
    if (p->realParams.contains("s_13")) {
        return (p->realParams.at("s_13")->get() > s0 - ds) && (p->realParams.at("s_13")->get() < s0 + ds);
    } else {
        return (p->realParams.at("s_14")->get() > s0 - ds) && (p->realParams.at("s_14")->get() < s0 + ds);
    }
    // return (p->s_13 > s0 - ds) && (p->s_13 < s0 + ds);
}
...
double KinematicsCalculator::Beta(double x, double y, double z) {
...
void KinematicsCalculator12::initialize_constants() {
    ...
    double b23 = Beta(m_masses_sq[1], m_masses_sq[2], m_s);
    double p = m_1 * b23 / 2;
...
void KinematicsCalculator13::compute_momenta(std::vector<double> kin_params) {
    ...
    double cos_s = (4 * m_masses_sq[0] * kin_params[1] * t - (t + m_masses_sq[2] - m_masses_sq[3]) * (t + m_masses_sq[0] - m_masses_sq[1])) 
                        / (m_masses_sq[0] * t * Beta(m_masses_sq[1], t, m_masses_sq[0]) * Beta(m_masses_sq[2], m_masses_sq[3], t));
    double sin_s = sqrt(1 - cos_s * cos_s);
    double p = m_masses[0] * Beta(t, m_masses_sq[1], m_s) / 2;
    double p_star = std::sqrt(t) * Beta(m_masses_sq[2], m_masses_sq[3], t) / 2;
...
std::vector<std::pair<double, double>> KinematicsCalculator13::compute_kinematic_limits() const {
    ...
    if (m_masses[2] == 0 && m_masses[3] == 0) {
        ...
        y_min = (t_y_min + m_masses_sq[2] - m_masses_sq[3]) * (t_y_min + m_masses_sq[0] - m_masses_sq[1]) / (4 * t_y_min * m_masses_sq[0])
                            - Beta(m_masses_sq[1], t_y_min, m_masses_sq[0]) * Beta(m_masses_sq[2], m_masses_sq[3], t_y_min) / 4;
        y_max = (t_y_max + m_masses_sq[2] - m_masses_sq[3]) * (t_y_max + m_masses_sq[0] - m_masses_sq[1]) / (4 * t_y_max * m_masses_sq[0])
                            + Beta(m_masses_sq[1], t_y_max, m_masses_sq[0]) * Beta(m_masses_sq[2], m_masses_sq[3], t_y_max) / 4;
    }
...
void KinematicsCalculator22::initialize_constants() {
    KinematicsCalculator::initialize_constants();
    double b12 = Beta(m_masses_sq[0], m_masses_sq[1], m_s);
    double b34 = Beta(m_masses_sq[2], m_masses_sq[3], m_s);
...
void KinematicsCalculator23::compute_momenta(std::vector<double> kin_params) {
    ...
    double p      = std::sqrt(m_s) * Beta(t, m_masses_sq[2], m_s) / 2;
    double p_star = std::sqrt(t) * Beta(m_masses_sq[3], m_masses_sq[4], t) / 2;
...
double KinematicsCalculator23::compute_phase_space_factor(std::vector<double> kin_params) {
    double b3 = Beta(kin_params[0], m_masses_sq[2], m_s);
    double b45 = Beta(m_masses_sq[3], m_masses_sq[4], kin_params[0]);
